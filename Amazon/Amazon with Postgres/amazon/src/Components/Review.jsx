...
class Review extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      review: {
        comment: "",
        rate: "",
        elementid: parseInt(this.props.id),
      },
      editMode: false,
    };
  }
  componentDidMount = async () => {
    let response = await fetch(
      `http://127.0.0.1:3006/reviews/${this.props.id}`
    );
  ...
  sendReview = async () => {
    if (this.state.editMode === false) {
      let response = await fetch(`http://127.0.0.1:3006/reviews/`, {
        method: "POST",
      ...
      let response = await fetch(
        `http://127.0.0.1:3006/reviews/${this.state.review.id}`,
        {
          method: "PUT",
  ...
  removeReview = async (reviewId) => {
    let response = await fetch("http://localhost:3006/reviews/" + reviewId, {
      method: "DELETE",
    });
    if (response.ok) {
                ...
                this.state.reviews.map((review) => {
                  return (
                    <li>
                      ...
                      <span>
                        {" "}
                        {review.comment} by {review.elementid}
                      </span>
                      <Dropdown>
                ...
                <Form>
                  <Form.Row>
                    ...
                    <Form.Group as={Col} sm={2}>
                      ...
                      <Form.Control
                        onChange={this.updateReview}
                        defaultValue={this.state.review.rate}
                        id="rate"
                        as="select"
                        defaultValue="1"
                      >
